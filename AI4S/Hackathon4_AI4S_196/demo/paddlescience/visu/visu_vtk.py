...
def __save_vtk_modified(filename="output", cordinate=None, title=None, data=None):
    npoints = len(cordinate)
    ndims = len(cordinate[0])
    ...
    if ndims == 3:
        ...
        axis_y = cordinate[:, 1].copy().astype(config._dtype)
        axis_z = np.zeros(npoints, dtype=config._dtype)
        pointsToVTK("results/"+filename, axis_x, axis_y, axis_z, data=data_vtk)
