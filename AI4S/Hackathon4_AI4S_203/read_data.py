...
def LoaderNormalizer(data, isTest=False, shuffle=0, dataProp=None):
    ...
    if dataProp is None:
        ...
        data.inputs = np.empty((len(files), 3, 128, 128), dtype=np.float32)
        data.targets = np.empty((len(files), 3, 128, 128), dtype=np.float32)
        ...
        data.inputs = np.empty((data.totalLength, 3, 128, 128), dtype=np.float32)
        data.targets = np.empty((data.totalLength, 3, 128, 128), dtype=np.float32)
...
class TurbDataset(Dataset):
    ...
    def __init__(self, dataProp=None, mode=TRAIN, dataDir="../data/train/", dataDirTest="../data/test/", shuffle=0,
                 normMode=0):
