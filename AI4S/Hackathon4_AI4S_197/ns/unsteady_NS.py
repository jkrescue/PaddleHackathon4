...
print_freq: int = int(args.print_freq)
save_freq: int = int(args.save_freq)
...
class PINN_NS_unsteady(DeepModelSingle):
    ...
    def plot_result(self, p_pre, u_pre, v_pre, filename):
        ...
        im = ax[3].pcolormesh(x.ravel().reshape(*_shape), y.ravel().reshape(*_shape), p_pre.ravel().reshape(*_shape),
                              shading='gouraud')
        ...
        im = ax[5].pcolormesh(x.ravel().reshape(*_shape), y.ravel().reshape(*_shape),
                              p_pre.ravel().reshape(*_shape) - p.ravel().reshape(*_shape), cmap='bwr',
                              shading='gouraud')
...
def build(model, x_train, N_truth, loss_type, weight):
    ...
    if loss_type == "square":
        u_loss = paddle.norm(u_tra - Tra_out[:, 0:1], p=2) ** 2 / u_tra.shape[0]
        v_loss = paddle.norm(v_tra - Tra_out[:, 1:2], p=2) ** 2 / v_tra.shape[0]
    elif loss_type == "l1":
        u_loss = paddle.norm(u_tra - Tra_out[:, 0:1], p=1) / u_tra.shape[0]
        v_loss = paddle.norm(v_tra - Tra_out[:, 1:2], p=1) / v_tra.shape[0]
    else:
    ...
    return [P_pre, U_pre, V_pre], [data_loss, eqs_loss, loss_batch], Scheduler


def run_experiment(epoch_num, noise_type, noise, loss_type, weight, N=5000, _data=[], l_size=-1, abnormal_size=0):
    print(f'\n{epoch_num}_{loss_type}_{N}_{noise_type}_{noise}_{abnormal_size}_{weight}')
    try:
    ...
    [P_pre, U_pre, V_pre], Loss, Scheduler = build(Net_model, x_train, N_truth, loss_type, weight)
    ...
    for epoch in range(start_epoch, 1 + adam_iter):
        ...
        if epoch > 0 and epoch % print_freq == 0:
            ...
            plt.savefig(
                path.joinpath(f'{epoch_num}_{loss_type}_{N}_{noise_type}_{noise}_{abnormal_size}_{weight}_loss.png'))
        if epoch > 0 and epoch % save_freq == 0:
            ...
            paddle.save(prog, os.path.join(path,
                                           f'{epoch_num}_{loss_type}_{N}_{noise_type}_{noise}_{abnormal_size}_{weight}.pdmodel'))
